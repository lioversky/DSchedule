...
import com.chanct.schedule.bean.IScheduleTaskDealMulti;
...
import com.chanct.schedule.util.LockObject;
           ...
           //在每次数据处理完毕后休眠固定的时间
			...
			if (taskItems.size() > 0) {
				List<T> tmpList = this.taskDealBean.selectTasks(
						...
						this.scheduleManager.getTaskItemCount(), (List<T>) taskItems,
						taskTypeInfo.getFetchDataNumber());
				scheduleManager.getScheduleServer().setLastFetchDataTime(new Timestamp(scheduleManager.getScheduleCenter().getSystemTime()));
				if(tmpList != null){
		...
		try {
			...
			while (true) {
				...
				while (true) {
					...
					try {// 运行相关的程序
						startTime = scheduleManager.getScheduleCenter().getSystemTime();
						if (this.isMutilTask == false) {
							if (((IScheduleTaskDealSingle) this.taskDealBean).execute(executeTask, scheduleManager.getScheduleServer().getOwnSign()) == true) {
								addSuccessNum(1, scheduleManager.getScheduleCenter().getSystemTime() - startTime, "com.taobao.pamirs.schedule.TBScheduleProcessorSleep.run");
							} else {
								addFailNum(1, scheduleManager.getScheduleCenter().getSystemTime() - startTime, "com.taobao.pamirs.schedule.TBScheduleProcessorSleep.run");
							}
						} else {
							if (((IScheduleTaskDealMulti) this.taskDealBean).execute((Object[]) executeTask, scheduleManager.getScheduleServer().getOwnSign()) == true) {
								addSuccessNum(((Object[]) executeTask).length, scheduleManager.getScheduleCenter().getSystemTime() - startTime, "com.taobao.pamirs.schedule.TBScheduleProcessorSleep.run");
							} else {
								addFailNum(((Object[]) executeTask).length, scheduleManager.getScheduleCenter().getSystemTime() - startTime, "com.taobao.pamirs.schedule.TBScheduleProcessorSleep.run");
							}
						}
					} catch (Throwable ex) {
						if (this.isMutilTask == false) {
							addFailNum(1, scheduleManager.getScheduleCenter().getSystemTime() - startTime, "TBScheduleProcessor.run");
						} else {
							addFailNum(((Object[]) executeTask).length, scheduleManager.getScheduleCenter().getSystemTime() - startTime, "TBScheduleProcessor.run");
						}
