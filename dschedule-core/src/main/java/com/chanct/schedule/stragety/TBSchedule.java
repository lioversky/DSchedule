...
import com.chanct.schedule.entity.strategy.ScheduleStrategyRunntime;
import com.chanct.schedule.stragety.task.AssignFactoryTimerTask;
import com.chanct.schedule.stragety.task.ManagerFactoryTimerTask;
import com.chanct.schedule.taskmanager.TBScheduleManagerGroup;
import com.chanct.schedule.taskmanager.TBScheduleManagerStatic;
import com.chanct.schedule.util.ScheduleUtil;
import com.chanct.schedule.watcher.CuratorCacheWatcher;
...
//		this.factory.getScheduleStrategyManager().watchStrategy(new ManagerWatcher(this.factory.getZkManager(), this));
	...
	public void refresh() throws Exception {
		...
		try {
			...
			try {
				stsInfo = this.factory.getScheduleFactoryManager().loadManagerFactoryInfo(this.factory.getFactoryInfo().getUuid());
			} catch (Exception e) {
				isException = true;
			...
			if (isException == true) {
				try {
					...
					this.factory.getScheduleStrategyManager().unRregisterStrategyFactory(this.factory.getFactoryInfo());
				} finally {
...
//		停止当前不在本机运行的策略
		List<String> stopList = this.factory.getScheduleStrategyManager().loadAllNotScheduleStrategyRunntimeByUUID(this.factory.getFactoryInfo().getUuid());
...
//		查找factory注册到的所有策略
		for (ScheduleStrategyRunntime run : this.scheduleStrategyManager.loadAllScheduleStrategyRunntimeByUUID(this.factory.getFactoryInfo().getUuid())) {
...
//			如果此策略下没有注册的factory，或者当前factory不是leader，直接返回不做后续处理
			if (factoryList.size() == 0 || this.isLeader(this.factory.getFactoryInfo().getUuid(), factoryList) == false) {
				continue;
			}
			// 加载策略信息
...
//			在strategy/factory里写入任务信息
	...
	public void assignScheduleFactory() throws Exception {
		...
		try {
			...
			if (this.factory.getScheduleFactoryManager().isLeader(this.factory.getFactoryInfo().getUuid(), factoryUUIDList)) {
...
//		按每台机器可以再分的任务数，本次要分配的总数，每台机器最大执行数，来计算每台机器可以对此策略再分配多少任务数
	...
	public void reRunScheduleServer() throws Exception {
		...
		ManagerFactoryInfo info = this.factory.getScheduleFactoryManager().loadManagerFactoryInfo(this.factory.getFactoryInfo().getUuid());
		int runs = 0;
//		查找当前factory注册到的所有策略
		List<ScheduleStrategyRunntime> runsList = this.scheduleStrategyManager.loadAllScheduleStrategyRunntimeByUUID(this.factory.getFactoryInfo().getUuid());
		for (ScheduleStrategyRunntime run : runsList) {
...
//			String ownSign = ScheduleUtil.splitOwnsignFromTaskType(strategy.getTaskName());
			result = new TBScheduleManagerStatic(factory,  strategy, factory.getScheduleDataManager());
		} else if (ScheduleStrategy.Kind.Java == strategy.getKind()) {
			result = (IStrategyTask) Class.forName(strategy.getTaskName()).newInstance();
			result.initialTaskParameter(strategy.getStrategyName(), strategy.getExecuteStrategy());
...
//			分配属于此线程的组内任务
//			TODO: 按策略
			result = new TBScheduleManagerGroup(factory, strategy, factory.getScheduleDataManager(),factory.getScheduleRuntimeManager());
		}
		return result;
	}
	/**
	...
	public void stopAll() throws Exception {
		try {
			...
			this.factory.getFactoryInfo().setUuid(null);
			logger.warn("停止服务成功！");
		} catch (Throwable e) {
	...
	private Properties loadProperties() throws Exception {
		...
		try {
			is = TBSchedule.class.getClassLoader().getResourceAsStream("pamirsScheduleConfig.properties");
			p.load(is);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
