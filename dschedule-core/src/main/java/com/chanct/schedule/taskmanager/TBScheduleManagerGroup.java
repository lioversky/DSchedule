 ...
 * 如果任务类型，组名相同的在同一组下
 * 
 * 在type下创建groupItem，再下级节点为任务名称
 * 
 * 
 * 进入类后，先调用assign方法分配item
 * 
 * 1.先创建线程个数的server注册到组任务下
 * 2.将server均分配到对应的item下
 * 3.server根据uuid加载对应的item
 * 4.启动执行
 * @author lihx
 *
 */
public class TBScheduleManagerGroup extends TBScheduleManager {
	...
	public TBScheduleManagerGroup(TBScheduleFactory aFactory, ScheduleStrategy strategy, IScheduleDataManager scheduleDataManager,IScheduleRuntimeManager sRuntimeManager) throws Exception {
		super(aFactory, strategy, scheduleDataManager);
		// 初始化ScheduleServer,每个线程一个server
...
//		this.taskTypeInfo.setPermitRunStartTime(strategy);
	...
	private void loadTask() throws Exception {
		while (this.taskTypeInfo == null) {
			...
			log.info(this.currenScheduleServer.getUuid() + " load task again after 1 seconds!");
			Thread.sleep(1000);
		}
	}
