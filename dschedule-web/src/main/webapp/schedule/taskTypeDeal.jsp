...
<%@page import="com.chanct.schedule.entity.manager.ScheduleTaskType"%>
...
<%
	...
	try {
		if (action.equalsIgnoreCase("createTaskType")||action.equalsIgnoreCase("editTaskType")) {
			...
			taskType.setKind(ScheduleTaskType.Kind.valueOf(request.getParameter("processType")));
			...
			taskType.setGroupItemName(request.getParameter("groupItemName"));
			taskType.setGroupIndex(request.getParameter("groupIndex")==null?0:Integer.parseInt(request.getParameter("groupIndex")));
...
//			taskType.setTaskItems(ScheduleTaskType.splitTaskItem(itemDefines));
			...
			if(action.equalsIgnoreCase("createTaskType")){
				if(ScheduleTaskType.Kind.Group==taskType.getKind()) {
					ConsoleManager.getScheduleDataManager().createGroupTaskType(taskType);
				}else {
