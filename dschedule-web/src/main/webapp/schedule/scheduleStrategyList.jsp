...
<%@page import="com.chanct.schedule.util.ScheduleUtil"%>
...
%>
     <tr onclick="openDetail(this,'<%=scheduleStrategy.getStrategyName()%>')">
		...
		<%}%>
     	...
     	<td><%=scheduleStrategy.getPermitRunStartTime()%></td>
     	<td><%=scheduleStrategy.getPermitRunEndTime()%></td>
     	<td><%=scheduleStrategy.getExecuteType()%></td>
     	<td><%=scheduleStrategy.getExecuteStrategy()%></td>
