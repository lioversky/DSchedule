...
<tr>
	<td>任务标志:</td>
	<td><input type="text" id="ownSign" name="ownSign"  value="<%=scheduleStrategy.getOwnSign()%>" width="30"></td>
	<td>区分用户标志</td>
</tr>
<tr>
	<td>执行开始时间：</td><td><input type="text" name="permitRunStartTime" value="<%=scheduleStrategy.getPermitRunStartTime()==null?"":scheduleStrategy.getPermitRunStartTime()%>" width="30"></td>
	
</tr>
<tr>
	<td>执行结束时间：</td><td><input type="text" name="permitRunEndTime" value="<%=scheduleStrategy.getPermitRunEndTime()==null?"":scheduleStrategy.getPermitRunEndTime()%>" width="30"></td>
</tr>
<tr>
	<td>执行策略类型：</td>
	<td><input type="text" name="executeType" value="<%=scheduleStrategy.getExecuteType()==null?"":scheduleStrategy.getExecuteType()%>" width="30"></td>
	<td>可先类型:1.每次启动执行一次StartRunEveryTime,2.按次数执行RunByNums,3.按表达式CronExp。执行策略值为对应数据值</td>
</tr>
<tr>
	<td>执行策略值：</td><td><input type="text" name="executeStrategy" value="<%=scheduleStrategy.getExecuteStrategy()==null?"":scheduleStrategy.getExecuteStrategy()%>" width="30"></td>
</tr>
<%-- <tr>
	<td>任务参数:</td>
