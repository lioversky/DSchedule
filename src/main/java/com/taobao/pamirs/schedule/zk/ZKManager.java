...
public class ZKManager{
	...
	private void createZookeeper(final CountDownLatch connectionLatch) throws Exception {
		zk = new ZooKeeper(this.properties.getProperty(PropertiesKeys.zkConnectString
				.toString()), Integer.parseInt(this.properties
				.getProperty(PropertiesKeys.zkSessionTimeout.toString())),
				new Watcher() {
					public void process(WatchedEvent event) {
						sessionEvent(connectionLatch, event);
					}
				});
		String authString = this.properties.getProperty(PropertiesKeys.userName.toString())
				+ ":"+ this.properties.getProperty(PropertiesKeys.password.toString());
		this.isCheckParentPath = Boolean.parseBoolean(this.properties.getProperty(PropertiesKeys.isCheckParentPath.toString(),"true"));
...
//		result.setProperty(keys.zkConnectString.toString(),"chanct01:2181");
